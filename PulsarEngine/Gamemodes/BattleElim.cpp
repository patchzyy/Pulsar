...
kmCall(0x8058CB7C, ForceInvisible);
...
kmRuntimeUse(0x80539878);  // BattleElimScore store
void BattleElim() {
    ...
    if ((RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_FROOM_HOST || RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_FROOM_NONHOST || RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_NONE) && (mode == MODE_BATTLE || mode == MODE_PRIVATE_BATTLE || mode == MODE_PUBLIC_BATTLE) && System::sInstance->IsContext(PULSAR_TEAM_BATTLE) == BATTLE_TEAMS_DISABLED) {
        isElim ? RACESETTING_ELIMINATION_ENABLED : RACESETTING_ELIMINATION_DISABLED;
    } else {
        isElim = RACESETTING_ELIMINATION_DISABLED;
    }
    if ((isElim == RACESETTING_ELIMINATION_ENABLED || (RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_BT_REGIONAL && System::sInstance->netMgr.region == 0x0F)) && RKNet::Controller::sInstance->roomType != RKNet::ROOMTYPE_VS_REGIONAL && RKNet::Controller::sInstance->roomType != RKNet::ROOMTYPE_VS_WW) {
